cat > setup-dual-stack-proxy.sh << 'EOF'
#!/bin/bash
...
NDPEOF
    ...
    if [ ! -f "$BINARY" ]; then
        wget -q https://github.com/Mrio233/go-ipv6-pool/releases/latest/download/$BINARY -O /opt/$BINARY
        chmod +x /opt/$BINARY
    fi
...
chmod +x setup-dual-stack-proxy.sh && bash setup-dual-stack-proxy.sh
